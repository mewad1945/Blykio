...
def super_intensiv_analys():
    ...
    # --- VARIABLER FÖR GOBLIN_DRAGONALT ---
    goblin_transaktioner = []
    total_lagliga_pengar = 0
    total_olagliga_pengar = 0
    goblin_har_saljt = False
    ...
    for i, rad in enumerate(rader):
        ...
        # =================================================================
        # DJUPANALYS: GOBLIN_DRAGONALT (KOLLA HANS /SELL OCH EKONOMI)
        # =================================================================
        if "goblin_dragonalt" in r_lower:
            # 1. Kolla om han startade en försäljning
            if "sell" in r_lower:
                goblin_har_saljt = True
                    ...
                    if i + j < len(rader):
                        ...
                        if "eco give" in nasta_rad or "money give" in nasta_rad or "eco:give" in nasta_rad:
                            ...
                            if pengar_match:
                                ...
                                goblin_transaktioner.append(f"LAGLIGT: Sålde {block_namn} och fick ${summa:.2f} via /sell")
                                break

            # 2. Kolla om han fick pengar UTAN att ha kört /sell (Direkt fusk/insättning)
            if ("eco give" in r_lower or "money give" in r_lower or "pay" in r_lower) and "goblin_dragonalt" in r_lower:
                # Kontrollera om han körde /sell precis innan, annars är det olagligt
                körde_sell_innan = False
                for k in range(max(0, i-4), i):
                    if "sell" in rader[k].lower() and "goblin_dragonalt" in rader[k].lower():
                ...
                if not körde_sell_innan:
                    ...
                    if pengar_match:
                        ...
                        goblin_transaktioner.append(f"OLAGLIGT: Fick ${s:.2f} direkt via kommando/insättning (Ingen /sell hittad!)")
    ...
    # Slutsats Goblin_Dragonalt
    print("\n[📊 GOBLIN_DRAGONALT EKONOMI-DOM]:")
    ...
    elif len(goblin_transaktioner) == 0:
        print("  ⚪ STATUS: Inga spår av pengatransaktioner hittades för Goblin_Dragonalt i denna fil.")
    else:
        print(f"  ⚠️ STATUS: Okänd hantering (Totalt: {total_lagliga_pengar + total_olagliga_pengar:.2f} kr)")

    if goblin_transaktioner:
        print("  Ekonomiska händelser (Max 5 visas):")
        for t in goblin_transaktioner[:5]:
            print(f"    -> {t}")
